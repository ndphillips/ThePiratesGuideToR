...
```{r, fig.cap= "", fig.margin = TRUE, echo = FALSE, out.width = "20%"}
...
```{r rscreenshot, fig.cap= "{Here is how the base R application looks. While you can use the base R application alone, most people I know use RStudio -- software that helps you to write and use R code more efficiently!", fig.margin = TRUE, echo = FALSE, out.width = "50%", fig.align ="center"}
...
```{r rstudiowindows, fig.cap= "The four panes of RStudio.", fig.margin = TRUE, echo = FALSE, out.width = "80%"}
...
```{r sourcewindow, fig.cap= "The Source contains all of your individual R scripts. The code won't be evaluated until you send it to the Console.", fig.margin = TRUE, echo = FALSE, out.width = "75%"}
...
meeen(data)
```
<div class="error">Error: cound not find function "meeen"</div>
...
If you include an extra space in the middle of the name of an object or function, you'll receive an error. In the code below, I'll accidently write `Chick Weight` instead of `ChickWeight`:
